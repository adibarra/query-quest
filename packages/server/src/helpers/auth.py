...
# @description: Auth helper class for user authentication and authorization
...
class Auth:
    ...
    def validate_username(username: str) -> bool:
        ...
        if len(username) < Auth.USERNAME_LENGTH_MIN:
            raise ValueError(Auth.ERROR_MESSAGES[Auth.ERROR_TYPES.USERNAME_SHORT])

        if len(username) > Auth.USERNAME_LENGTH_MAX:
            raise ValueError(Auth.ERROR_MESSAGES[Auth.ERROR_TYPES.USERNAME_LONG])

        for char in username:
            if char not in Auth.USERNAME_VALID_CHARACTERS:
                raise ValueError(
                    Auth.ERROR_MESSAGES[Auth.ERROR_TYPES.USERNAME_INVALID_CHARACTERS]
                )
    ...
    def validate_email(email: str) -> bool:
        ...
        if Auth.EMAIL_REGEX.match(email) is None:
            raise ValueError(Auth.ERROR_MESSAGES[Auth.ERROR_TYPES.EMAIL_INVALID])
    ...
    def validate_password(password: str) -> bool:
        ...
        if len(password) < Auth.PASSWORD_LENGTH_MIN:
            raise ValueError(Auth.ERROR_MESSAGES[Auth.ERROR_TYPES.PASSWORD_SHORT])
